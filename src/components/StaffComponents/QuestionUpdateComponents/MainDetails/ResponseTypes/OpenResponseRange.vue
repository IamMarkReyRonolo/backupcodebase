			...
			<div class="optionContent">
				<div class="option">
					...
					<v-text-field
						...
						row-height="20"
						ref="textarea1"
						v-model="questionStore.updatedQuestion.options[0].content"
						:rules="[lessThanRule]"
						type="number"
				...
				<div class="option">
					...
					<v-text-field
						...
						row-height="20"
						ref="textarea2"
						v-model="questionStore.updatedQuestion.options[1].content"
...
<script>
	...
	export default {
		...
		created() {
			...
			this.questionStore.updatedQuestion.options[0].is_answer = true;
			this.questionStore.updatedQuestion.options[1].is_answer = true;
			this.questionStore.updatedQuestion.options[0].image = "";
			this.questionStore.updatedQuestion.options[1].image = "";
			if (
				!!isNaN(Number(this.questionStore.updatedQuestion.options[0].content))
			) {
				this.questionStore.updatedQuestion.options[0].content = "";
			}

			if (
				!!isNaN(Number(this.questionStore.updatedQuestion.options[1].content))
			) {
				this.questionStore.updatedQuestion.options[1].content = "";
			}

			this.option_unit = this.questionStore.updatedQuestion.options[0].unit;
		},
		data: () => ({
		...
		computed: {
			lessThanRule: function () {
				try {
					let result;
					if (this.questionStore.updatedQuestion.options[0].content) {
						let num1 = parseFloat(
							this.questionStore.updatedQuestion.options[0].content
						);
						let num2 = parseFloat(
			...
			greaterThanRule: function () {
				try {
					let result;
					if (this.questionStore.updatedQuestion.options[1].content) {
						let num1 = parseFloat(
							this.questionStore.updatedQuestion.options[0].content
						);
						let num2 = parseFloat(
		...
		watch: {
			option_unit() {
				this.questionStore.updatedQuestion.options[0].unit = this.option_unit;
				this.questionStore.updatedQuestion.options[1].unit = this.option_unit;
			},
		},
	};
</script>
