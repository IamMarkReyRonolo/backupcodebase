	...
	<div>
		...
		<div class="field">
			...
			<v-text-field
				filled
				dense
				v-model="questionStore.createQuestion.metadata.test_code"
				:rules="required"
				counter="6"
				id="setTestCode"
			></v-text-field>
	...
	export default {
		created() {
			...
			this.typeOfQ.forEach((tq) => {
				if (
					tq.name == this.questionStore.createQuestion.metadata.classification
				) {
					tq.selected = true;
			...
			if (!hasSelected) {
				...
				this.questionStore.createQuestion.metadata.classification =
					this.typeOfQ[0].name;
