...
<script>
	...
	export default {
		...
		async created() {
			...
			for (let i = 0; i < this.questionStore.createQuestion.options; i++) {
				if (this.questionStore.createQuestion.options[i].is_answer) {
					this.radioGroup = this.questionStore.createQuestion.options[i].letter;
					break;
				}
			}
		},
